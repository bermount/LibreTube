...
import android.util.SparseBooleanArray // Using this for active downloads
...
    // Tracks actively running downloads by their IDs
    private val activeDownloads = SparseBooleanArray()
...
        activeDownloads[item.id] = true // Mark this download as active
...
        while (totalRead < item.downloadSize && activeDownloads[item.id]) { // Continue only if active
            try {
                totalRead = progressDownload(item, url, totalRead, notificationBuilder)
            } catch (_: CancellationException) {
                Log.d(this@DownloadService::class.java.name, "Download ${item.id} cancelled.")
                break
            } catch (e: Exception) {
...
        // Determine final status and update notification
        val completed = totalRead >= item.downloadSize && activeDownloads[item.id] // Only completed if it wasn't paused/stopped
        if (completed) {
            _downloadFlow.emit(item.id to DownloadStatus.Completed)
            setPauseNotification(notificationBuilder, item, true) // Pass true for completed
        } else if (activeDownloads[item.id]) { // If still marked active but not completed, it must have paused
            _downloadFlow.emit(item.id to DownloadStatus.Paused)
            setPauseNotification(notificationBuilder, item, false) // Pass false for paused
        } else { // It was stopped, notification handled in stop()
            // Do nothing, notification already cancelled in stop()
        }

        // Remove from active downloads
        activeDownloads.remove(item.id)

        // After a download finishes (either completed or paused by internal logic), try to start the next one
...
        while (activeDownloads[item.id] && sourceByte // Check activeDownloads here
...
        return activeDownloads.size() < DownloadHelper.getMaxConcurrentDownloads()
    }
...
                downloadFile(item)
            } else {
                // If cannot start, implicitly pause the download by not calling downloadFile
                // and emit a paused status so UI can reflect it.
                _downloadFlow.emit(item.id to DownloadStatus.Paused)
            }
        }
    }
...
     */
    fun pause(id: Int) {
        activeDownloads.remove(id) // Mark as not active, will stop the while loop in progressDownload
...
        activeDownloads.remove(id) // Mark as not active
...
    /**
     * Stop service if no downloads are active or pending.
     */
    private fun stopServiceIfDone() {
        lifecycleScope.launch(coroutineContext) {
            val hasAnyPendingOrActiveDownloads = activeDownloads.size() > 0 || Database.downloadDao().getAllDownloadItems().any { it.status == DownloadStatus.Paused || it.status == DownloadStatus.InProgress }
            if (!hasAnyPendingOrActiveDownloads) {
...
    /**
     * Attempts to start the next pending (paused) download if there's capacity.
     */
    private fun tryStartNextDownload() {
        lifecycleScope.launch(coroutineContext) {
            if (mayStartNewDownload()) {
                val nextPausedDownload = Database.downloadDao().getAllDownloadItems().firstOrNull { it.status == DownloadStatus.Paused }
                if (nextPausedDownload != null) {
                    resume(nextPausedDownload.id) // Resume the found download
                } else {
                    stopServiceIfDone() // If no more pending downloads, check if service can stop
                }
            } else {
                stopServiceIfDone() // If no more capacity, check if service can stop (though it shouldn't if active downloads exist)
            }
        }
    }
...
        super.onBind(intent)
        // When bound, resume any downloads that were in progress or paused.
        lifecycleScope.launch(coroutineContext) {
            Database.downloadDao().getAllDownloadItems().forEach { downloadItem ->
                // Check if the item is currently active (being downloaded)
                val isCurrentlyActive = activeDownloads[downloadItem.id]
                // Resume if it's not active but should be (e.g., app was killed, service recreated)
                if (!isCurrentlyActive && (downloadItem.status == DownloadStatus.InProgress || downloadItem.status == DownloadStatus.Paused)) {
                    resume(downloadItem.id)
                }
            }
            // After resuming potential items, try to start the next ones if capacity allows
            tryStartNextDownload()
        }
        return binder
    }
